...
2. **Terraform plan** (`terraform-plan.yml`), automático somente nos Pull Requests que
   alteram a infraestrutura. Usa environments sem reviewers e nunca executa apply.
3. **Terraform deploy** (`terraform-apply.yml`), iniciado por push em `homolog` ou
   ...
   executa o plan autoritativo, apply e sincronização do JDBC com Auth e Backend.
   Homologação não possui gate humano; produção concentra uma única aprovação no GitHub
   Environment `production`.

O `workflow_dispatch` do deploy permanece somente para repetir o fluxo durante bootstrap
ou recuperação, a partir da branch correspondente. Destroy é manual via Terraform CLI e
não faz parte do GitHub Actions. O Auto apply do workspace HCP permanece desligado.

## Consequências

- o merge em `homolog` inicia plan, apply e sincronização automaticamente, sem aprovações repetidas;
- o merge em `main` registra uma única aprovação antes do mesmo fluxo em produção;
- Auth e Backend recebem o JDBC no próprio run que cria ou atualiza o RDS;
